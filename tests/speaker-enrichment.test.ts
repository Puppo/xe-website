...
describe('speaker enrichment', () => {
  it('estrae solo foto associate a un singolo speaker', () => {
    ...
    expect(extractLegacySpeakerCandidates(html, 'https://www.xedotnet.org/eventi/test/')).toEqual([{
      name: 'Ada Lovelace',
  ...
  it('rifiuta loghi e seleziona solo candidati non ambigui', () => {
    ...
    expect(selectImageCandidate([{ imageUrl: 'https://example.com/a.jpg' }, { imageUrl: 'https://example.com/a.jpg' }])).toBe('https://example.com/a.jpg');
    expect(selectImageCandidate([{ imageUrl: 'https://example.com/a.jpg' }, { imageUrl: 'https://example.com/b.jpg' }])).toBeUndefined();
  });

  it('sostituisce solo gli scalari speaker e preserva il resto del frontmatter', () => {
    const source = 'title: Evento\nsessions:\n  - title: Talk\n    speakers:\n      - Ada Lovelace\n    description: Test';
    const result = replaceSpeakerScalars(source, new Map([['ada lovelace', 'ada-lovelace']]));
    expect(result).toBe('title: Evento\nsessions:\n  - title: Talk\n    speakers:\n      - person: ada-lovelace\n    description: Test');
  });
