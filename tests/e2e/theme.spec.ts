...
const themeLabels = { system: 'Sistema', light: 'Chiaro', dark: 'Scuro' } as const;

async function expectThemeIcon(control: Locator, preference: typeof themePreferences[number]) {
  await expect(control).toHaveAttribute('data-theme-preference', preference);
  await expect(control.locator('.theme-value')).toHaveText(themeLabels[preference]);
  for (const icon of themePreferences) {
    const expectation = expect(control.locator(`[data-theme-icon="${icon}"]`));
    if (icon === preference) await expectation.toBeVisible();
    else await expectation.toBeHidden();
  }
}
