...
const { site, base } = deploymentConfig(publicUrl);
const errors = [];

function outputPath(pathname) {
  const relativePath = decodeURIComponent(pathname.slice(base.length));
  if (!relativePath || relativePath.endsWith('/')) return join('dist', relativePath, 'index.html');
...
function inspectUrl(value, file, attribute) {
  if (!value || value.startsWith('#') || value.startsWith('mailto:') || value.startsWith('tel:') || value.startsWith('data:')) return;
  ...
  if (!url.pathname.startsWith(base)) {
    errors.push(`${file}: ${attribute} esce dal percorso base ${base}: ${value}`);
    return;
  }
  const target = outputPath(url.pathname);
  if (!existsSync(target)) errors.push(`${file}: destinazione mancante per ${attribute}=${value} (${target})`);
}
...
for (const file of htmlFiles) {
  ...
  $('[href], [src], [poster]').each((_index, element) => {
    for (const attribute of ['href', 'src', 'poster']) inspectUrl($(element).attr(attribute), file, attribute);
  });
}

const robots = readFileSync('dist/robots.txt', 'utf8');
const expectedSitemap = `${site}${base}sitemap-index.xml`;
if (!robots.includes(expectedSitemap)) errors.push(`robots.txt non contiene ${expectedSitemap}`);
...
if (errors.length) {
  ...
  if (errors.length > 30) console.error(`...e altri ${errors.length - 30} errori.`);
  process.exit(1);
}
